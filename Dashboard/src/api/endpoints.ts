...
function normalizeModules(value: unknown): ModuleModel[] {
  return asRecords(value).map((record) => ({
    id: numberValue(record, "id", "Id", "moduleId", "ModuleId", "module_id"),
    name: textValue(record, "name", "Name", "moduleName", "ModuleName", "module_name") ?? "",
...
export const userAccessRightsApi = {
  ...
  getByUser: async (userId: number) => normalizeAccess(await apiClient.get<unknown>(`/user-access-rights/by-user/${userId}`)),
