...
export const ModuleMasterPage: React.FC = () => {
  const { modules, addModule, updateModule, deleteModule, menuItems, getMessage } = useDatabase();
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingModule) {
      ...
      const newCode = `MOD00${modules.length + 1}`;
      addModule({
        code: newCode,
        name: name.trim(),
        description: description.trim(),
  ...
  const handleDelete = (m: Module) => {
    // Check if any menus are mapped under this module
    const mappedMenus = menuItems.filter(menu => menu.module.toLowerCase() === m.name.toLowerCase());
    if (mappedMenus.length > 0) {
      alert(`Cannot delete module "${m.name}" because it contains ${mappedMenus.length} menu items.`);
      return;
    }

    if (window.confirm(`Are you sure you want to delete module ${m.code}?`)) {
      deleteModule(m.code);
    }
  };
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {filteredModules.map(m => {
          const menuCount = menuItems.filter(item => item.module.toLowerCase() === m.name.toLowerCase()).length;
          return (
            <div 
              key={m.id}
              className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col justify-between shadow-sm hover:shadow-md transition-all"
            >
              <div>
                <div className="flex items-start justify-between">
                  ...
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-mono font-semibold ${
                    m.status === 'Active' ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-300' : 'bg-slate-100 text-slate-600 ring-1 ring-slate-200'
                  }`}>
                    {m.status}
                  </span>
                </div>
                ...
                <p className="text-xs text-slate-400 mt-2">
                  Menu Items: <strong className="text-slate-700">{menuCount}</strong>
                </p>
              </div>
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-xs text-slate-800 outline-none focus:border-blue-500 min-h-[60px]"
                  placeholder="Module purpose/announcements..."
                />
              </div>
